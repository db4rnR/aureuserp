...
function product_supplier_model_attributes_and_properties()
{
...
function product_supplier_model_relationships_with_other_models()
{
...
function product_supplier_model_date_range_functionality()
{
...
function product_supplier_model_with_multiple_suppliers_for_a_product()
{
    // Create a product
    $product = Product::factory()->create(['name' => 'Test Product']);
    ...
    $requiredQty = 25;
    $eligibleSuppliers = $productSuppliers->filter(function ($supplier) use ($requiredQty) {
        return $supplier->min_qty <= $requiredQty;
    });
