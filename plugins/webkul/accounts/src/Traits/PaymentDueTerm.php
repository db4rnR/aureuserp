...
trait PaymentDueTerm
{
    public function form(Form $form): Form
    {
        return $form
            ->components([
                Select::make('value')->options(DueTermValue::class)
                    ->label(__('accounts::traits/payment-due-term.form.value'))
                    ->required(),
                TextInput::make('value_amount')->label(__('accounts::traits/payment-due-term.form.due'))
                    ->default(100)
                    ->numeric()
                    ->minValue(0)
                    ->maxValue(99999999999),
                Select::make('delay_type')->options(DelayType::class)
                    ->label(__('accounts::traits/payment-due-term.form.delay-type'))
                    ->required(),
                TextInput::make('days_next_month')->default(10)
                    ->label(__('accounts::traits/payment-due-term.form.days-on-the-next-month')),
                TextInput::make('nb_days')->default(0)
                    ->numeric()
                    ->minValue(0)
                    ->maxValue(99999999999)
                    ->label(__('accounts::traits/payment-due-term.form.days')),
                Select::make('payment_id')->relationship('paymentTerm', 'name')
    ...
    public function table(Table $table): Table
    {
        return $table
            ->columns([
                TextColumn::make('value_amount')->label(__('accounts::traits/payment-due-term.table.columns.due'))
                    ->sortable(),
                TextColumn::make('value')->label(__('accounts::traits/payment-due-term.table.columns.value'))
                    ->formatStateUsing(fn ($state) => DueTermValue::options()[$state])
                    ->sortable(),
                TextColumn::make('value_amount')->label(__('accounts::traits/payment-due-term.table.columns.value-amount'))
                    ->sortable(),
                TextColumn::make('nb_days')->label(__('accounts::traits/payment-due-term.table.columns.after'))
                    ->sortable(),
                TextColumn::make('delay_type')->formatStateUsing(fn ($state) => DelayType::options()[$state])
